...
from types import FunctionType, BuiltinFunctionType, CodeType, GetSetDescriptorType, MappingProxyType, \
    MethodDescriptorType, ModuleType, WrapperDescriptorType, CellType
from lr2.parser.const import BYTES_TYPE, CODE_TYPE, FUNCTION_TYPE, CLASS_TYPE, INSTANCE_TYPE, MODULE_TYPE
...
class Serializer():
    ...
    def _serialize_class(self, _class: type) -> dict:
        result = {"__name__": _class.__name__, "members": {}}
        ...
        if _class == type:
            ...
            for member in inspect.getmembers(_class):
                if member[0] not in (
                        "__mro__", "__base__", "__class__", "__name__", "__qualname__", "__flags__"
    ...
    def _serialize_instance(self, instance: Any) -> dict:
        result = {"class": self._serialize_class(instance.__class__), "members": {}}
    ...
    def _deserialize_class(self, class_dict: dict) -> type:
        ...
        for member_key, member_value in class_dict["members"].items():
            ...
            if type(member_value) is dict and member_value["type"] == FUNCTION_TYPE:
